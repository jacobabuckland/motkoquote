  ...
  describe("Scenario 1: Bathroom refit", () => {
    ...
    it("draft stage produces expected line structure", async () => {
      ...
      const draft = await draftQuoteLineItems(
        ...
        {
          trade: null,
          // Deliberately the values the RECORDING was made with, not the
          // fixture's rates. The drafting model proposes STRUCTURE and code
          // computes every amount (see compile-draft.ts), so rates do not
          // belong in this call — and the recorder hashes the prompt, so
          // changing them here invalidates the recording and fails loudly
          // rather than replaying a response to a different question.
          //
          // KNOWN LIMITATION: the recording was also made with an empty team
          // roster, so the drafted labour line cannot reference the apprentice
          // the transcript describes. Re-recording under contractorContext is
          // a reviewed change (drafting temperature is 1.0, so a re-record is
          // not reproducible), not a refresh.
          day_rate: null,
          overtime_rate: null,
          callout_min: null,
          travel_rate: null,
          markup_pct: null,
          team_members: [],
          // The three database-backed lookups, stubbed EXPLICITLY. Each one
          // swallows its own failure and returns empty in production, so an
          // unstubbed harness replays a different pipeline than the real one —
          // green, deterministic and wrong.
          similar_past_jobs: [],
    ...
    it("compile stage produces expected line items", async () => {
      ...
      expect(
        failures,
        `The compiled quote does not match what the transcript says it should be:\n${describeFailures(failures)}`,
      ).toEqual([]);
    }, 30000);
