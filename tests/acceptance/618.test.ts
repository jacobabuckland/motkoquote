...
// The lookups mock `fetch`, but `authHeader()` reads the API key BEFORE any
// request is made and throws when it is absent — which is the correct
// fail-fast behaviour for a missing configuration value, and what QA required
// restoring on cycle 2. Without a key set here, two tests fail on
// "COMPANIES_HOUSE_API_KEY is not configured" before their mocked fetch is
// ever reached. Setting it makes the test's own environment coherent; it
// weakens no assertion, because every response is still the mock's.
const NO_KEY = Symbol("absent");
let previousKey: string | typeof NO_KEY = NO_KEY;

beforeEach(() => {
  previousKey = "COMPANIES_HOUSE_API_KEY" in process.env
    ? (process.env.COMPANIES_HOUSE_API_KEY as string)
    : NO_KEY;
  process.env.COMPANIES_HOUSE_API_KEY = "test-key";
});

afterEach(() => {
  if (previousKey === NO_KEY) delete process.env.COMPANIES_HOUSE_API_KEY;
