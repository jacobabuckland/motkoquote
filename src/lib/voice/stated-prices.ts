...
function detectQualifiers(
  ...
  return {
    // "each" is the obvious marker and was the only one. Trades say the unit
    // just as often — "eleven pounds a bag", "£14.50 per sheet" — and without
    // it a per-unit price was treated as a lump sum, so eight bags at £11
    // charged £11. The unit list is the one that already tells a quantity from
    // a price, so the two answers cannot disagree about what a unit is.
    //
    // A time rate is NOT a per-unit price and is deliberately not matched
    // here: "two fifty a day" is refused by containsRateUnit, which is the
    // right answer and a different one.
    each: /\beach\b/i.test(after) || PER_UNIT_AFTER_PRICE.test(after),
 ...
 */
/**
 * "…a bag", "…per sheet" — the unit named AFTER a price, which makes it a
 * per-unit price as surely as the word "each" does.
 */
const PER_UNIT_AFTER_PRICE =
  /^\s*(?:a|an|per)\s+(?:sq(?:uare)?\s*(?:m|metres?|meters?)|lin(?:ear)?\s*(?:m|metres?|meters?)|sqm|m2|metres?|meters?|kg|kilos?|kilograms?|tonnes?|tons?|litres?|liters?|bags?|sheets?|tubs?|tubes?|rolls?|boxes|box|packs?|bundles?|lengths?|coats?|slabs?|tiles?|panels?|doors?|windows?|sockets?|points?|radiators?)\b/i;
...
function extractBestMoneyPhrase(sentence: string): { phrase: string; startPos: number } | null {
  ...
  while (scanFrom < words.length) {
    ...
    // PER_UNIT_AFTER_PRICE — which is anchored on the article — cannot match.
