...
export const JobIntake = ({ adapter }: { adapter: JobIntakeAdapter }) => {
  ...
  const draftQuote = async () => {
    ...
    // A guest session has no key by design, so its absence is only a fault for
    // the account intake — which cannot draft without the row it wrote to.
    if (adapter.mode === "account" && !sessionKey) {
      stopRotatingMessages();
    ...
    try {
      ...
      adapter.reportFailure?.({ sessionKey, stage, message });
  ...
  const retry = () => {
    setError(null);
    setPipelineFailed(false);
    // Re-draft from what is already in refs. The account intake needs its row
    // id back; a guest has none, and its SoW lives in the ref, so the presence
    // of a transcript is the whole precondition there.
    const canRedraft =
      adapter.mode === "guest"
        ? transcriptRef.current.length > 0
        : Boolean(sessionKeyRef.current) && transcriptRef.current.length > 0;
    if (canRedraft) {
      void draftQuote();
    } else {
  ...
  const handleToolCall = async (name: string, callId: string, argsJson: string) => {
    const sessionKey = sessionKeyRef.current;
    const dc = dcRef.current;
    // A guest has no session key on purpose — only the account intake needs one
    // before it can fold a delta into its row.
    if (!dc || (adapter.mode === "account" && !sessionKey)) return;
    ...
    if (name === "update_sow") {
      ...
      try {
        updated = await adapter.persistDelta({
          sessionKey,
          current: sowStateRef.current,
          delta: parsedArgs,
        });
        setSowState(updated);
    ...
    if (name === "record_first_name") {
      ...
      sendToolAck(dc, callId, { ok: true });
      if (firstName) adapter.recordFirstName?.(firstName);
      sendResponse(dc);
      return;
    }
  ...
  // Typed-quote escape hatch. Only offered when the mode can provide one —
  // creating a typed draft needs somewhere to put it, so a guest is shown an
  // inline account prompt (adapter.manualUnavailable) at the point of use
  // instead, and is never navigated away from what they were doing.
  const goManual = adapter.startManual
    ? () => {
        setManualPending(true);
        void (async () => {
          try {
            await adapter.startManual?.();
          } catch (err) {
        ...
        <main className="flex flex-1 flex-col items-center justify-center gap-6 p-6">
          ...
          {micFailure ? (
            <MicFailureScreen
              ...
              manualSlot={adapter.manualUnavailable}
            />
          ) : (
            <MicExplainer
              ...
              manualLabel={goManual ? "Type the quote in instead" : undefined}
              manualPending={manualPending}
              // Deliberately NO account prompt here. This is the cold-launch
              // screen: nothing has been attempted yet, so there is nothing to
              // prompt about, and an unprompted "create an account" on the
              // first screen is the very thing this flow exists to remove. The
              // prompt appears on the mic-failure screen instead, where the
              // typed-quote alternative is actually being reached for.
            />
          )}
          ...
          {callState === "error" && pipelineFailed && (
            <div className="flex w-full flex-col items-center gap-4 text-center">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-xs text-text-secondary">{adapter.failureBody}</p>
              </div>
              <div className="flex w-full flex-col gap-2">
                <Button type="button" variant="primary" onClick={retry}>
                  Retry
                </Button>
                {adapter.saveForLater && (
                  <Button
                    type="button"
                    variant="secondary"
