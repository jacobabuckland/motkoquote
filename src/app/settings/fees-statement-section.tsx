...
export const FeesStatementSection = async ({ contractorId }: Props) => {
  ...
  const [{ data: accruedRows }, { data: collectionRows }] = await Promise.all([
    supabase
      .from("jobs")
  ...
  return (
    <section>
      ...
      <p className="mb-3 text-sm text-text-secondary">
        Our fee is £2 per paid job (£4 over £1,000), collected monthly by bank.
        The £2/£4 is VAT-inclusive — the net and VAT are shown below.
      </p>
      <Card className="space-y-4">
        <div>
          <p className="text-xs font-medium text-text-secondary">
            Accrued — to collect next
          </p>
