...
| **PFIX-5** | Redefine the invention-rate metric. The 55% is **WITHDRAWN** (areas/motko.md 2026-09-03). Replaced with a binary fixture-derived gate: against the fixture corpus, zero line items carry a monetary value absent from the transcript. Draft-vs-final cannot be used. `quote_line_edits` has four limits: written only on send; does not exclude null-mode collapses; matches on exact normalised description so a rename reads as remove+add; and its `normalize()` disagrees with the stored generated column (migration 14:17). | **Medium** — blocks any ship gate |
